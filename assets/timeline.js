$(function(){
	...
	var numDots = $(timeline).find('.month.dot').length + 1;
	var mobileYear = $(timeline).find('.phase.dot');
	...
	function mobileTimeline() {
		$(timelineDot).click(function(){
			...
			if ($(this).hasClass('phase')) {
				...
				$(this).prevAll('.dot.phase').addClass('complete');
				$('.dot.month').not(mobileQuarter).slideUp(500);
				$(mobileQuarter).slideDown(500);

			} else if ($(this).hasClass('month')) {

				var parentYear = $(this).prevUntil('.dot.phase');
